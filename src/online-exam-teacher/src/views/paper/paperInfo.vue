<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.paperName" placeholder="搜索试卷名称" clearable style="width: 200px;margin-right: 15px;"
                class="filter-item" @keyup.enter.native="handleFilter"/>
      <el-select v-model="listQuery.langId" placeholder="搜索科目" clearable style="width: 200px;margin-right: 15px;"
                 class="filter-item" @change="handleFilter">
        <el-option v-for="item in langOptions" :key="item.key" :label="item.label" :value="item.key"/>
      </el-select>
      <el-select v-model="listQuery.paperType" placeholder="搜索试卷类型" clearable style="width: 200px;margin-right: 15px;"
                 class="filter-item" @change="handleFilter">
    ...
    >
      ...
      <el-table-column label="操作" align="center" class-name="small-padding" width="189">
        <template slot-scope="{row}">
          <el-button v-waves type="primary" icon="el-icon-thumb" size="mini" @click="seePaperDetail(row)">
            预览
          </el-button>
    ...
    <el-dialog :visible.sync="paperDetailDialogFormVisible" :title="clickPaperTitle" style="margin-bottom: 20px">
      <el-button type="success" style="position: absolute;right: 20px;top: 12px;" @click="exportExam()">导出</el-button>
    ...
    <el-dialog :visible.sync="fixedDialogFormVisible" title="固定组卷" style="margin-bottom: 20px">
      <el-form ref="fixedDataForm" :rules="fixRules" :model="temp" label-position="left" label-width="100px"
        ...
        <el-form-item label="所属科目" prop="langId">
          <el-select v-model="temp.langId" placeholder="选择科目" clearable style="width: 200px;margin-right: 15px;"
...
export default {
  ...
  methods: {
    async exportExam(){
      ...
      console.log(row)
      let paperId=row.id
      let title=row.paperName
      let subTitle=row.langName
      let minutes=row.paperDuration/60
      let score=row.totalScore
      let singleScore=row.singleScore
      let multipleScore=row.multipleScore
      let judgeScore=row.judgeScore
      let fillScore=row.fillScore
      console.log(paperId+","+title+","+subTitle+","+minutes+","+score+","+singleScore+","+
      multipleScore+","+judgeScore+","+fillScore)
      let result=await reqExportPaper(paperId,title,subTitle,minutes,score,singleScore,multipleScore,judgeScore,fillScore)
